...
public Action SlayPlayerDelayed(Handle pTimer, int Client)
{
	if(Client > 0 && IsClientInGame(Client) && IsPlayerAlive(Client))
	{
		...
		PrintToChat(Client, " \x04 [NewVision]\x01 AirStuck Detected! You've got slayed.'");
		
		// Announce the other players
		PrintToChatAll(" \x07 [NewVision]\x01 Player\x06 %N\x01 was\x07 slayed\x01 because he tried to\x07 AirStuck\x01.", Client);
