...
stock bool:HandleRestrictionCommand(client, String:weapon[], team=0, amount=-1, bool:shouldbeall = false)
{
	...
	else if(!shouldbeall)
	{
		...
		{
			CReplyToCommand(client, "%T", "InvalidWeapon", client);
			return false;
		}
		if(amount != -1)
		{
...
public Action:RestrictAdminCmd(client, args)
{
	if(args < 1)
	{
		CReplyToCommand(client, "%T", "RestrictReply", client);
		return Plugin_Handled;
	}
	...
	if(args >= 2)
	{
		...
		{
			CReplyToCommand(client, "%T", "InvalidAmount", client);
			return Plugin_Handled;
		}
	}
	new teams = 0;
	if(args == 3)
	{
		...
		{
			CReplyToCommand(client, "%T", "InvalidTeam", client);
			return Plugin_Handled;
		}
	}
...
public Action:UnrestrictAdminCmd(client, args)
{
	if(args < 1)
	{
		CReplyToCommand(client, "%T", "UnrestrictReply", client);
		return Plugin_Handled;
	}
	decl String:weapon[100];
	GetCmdArg(1, weapon, sizeof(weapon));
...
public Action:DropC4(client, args)
{
	...
	CReplyToCommand(client, "%T", "NoOneHasBomb", client);
	return Plugin_Handled;
}
public Action:KnifeRound(client, args)
{
	...
	{
		CReplyToCommand(client, "%T", "SpecialRoundAlreadySet", client);
		return Plugin_Handled;
	}
